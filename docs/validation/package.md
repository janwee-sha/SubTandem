...
- `release-audit.json`：记录触发 commit、包内版本、精确大小、SHA-256、八项门禁、完整归档清单、两个 native helper、FFmpeg、正文路径与摘要及宿主覆盖状态。
...
`dist/native/` 必须只含 `subtandem-transport` 与 `subtandem-subtitle-extractor`。构建文件与包内文件必须分别包含 `arm64` 和 `x86_64`、最低 macOS 12、可执行权限、有效签名、相同 SHA-256，且只依赖系统动态库。
