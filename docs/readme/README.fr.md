...
- **Service de traduction au choix :** utilisez un endpoint compatible avec le contrat OpenAI Chat Completions, DeepSeek ou un serveur Ollama local/distant.
...
3. Dans **Translation service**, créez un Profile OpenAI, DeepSeek ou Ollama. Si le service exige une authentification, saisissez son API key avant d'actualiser manuellement la liste des modèles. Choisissez ensuite un modèle retourné ou saisissez un Model ID personnalisé exact.
...
- **Save** et **Test** ne sélectionnent pas le Profile et n'autorisent pas l'envoi des sous-titres. Cliquez explicitement sur **Select** ; auparavant, seule une requête de modèles sans sous-titres peut atteindre l'API root par défaut.
...
- Le transport helper inclus n'écoute que sur un port temporaire `127.0.0.1`. Un endpoint configuré ou en cours d'édition, y compris l'API root DeepSeek par défaut `https://api.deepseek.com`, peut recevoir une liste de modèles sans sous-titres avant Select ; seule la révision du Profile sélectionné reçoit le texte des sous-titres. Les redirect inter-origines et les identifiants inclus dans les URL sont refusés.
...
- **Translation service unavailable :** testez le Profile et vérifiez son endpoint, son Model ID exact, son API key, sa route réseau ou le processus Ollama. Pour DeepSeek, vérifiez aussi le solde, le quota, les limites de débit et l'accès à la route API fixe. La vidéo et les sous-titres d'origine continuent normalement.
