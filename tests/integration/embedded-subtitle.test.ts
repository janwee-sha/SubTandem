...
it("extracts real calibration media across four formats, metadata variants and exact streams before translating", async () => {
  const { readFileSync, readdirSync } = await import("node:fs");
  const { resolve } = await import("node:path");
  const { nativeLanguagePreparation } = await import("../helpers/native-language.js");
  const { LanguageDetectionCoordinator } = await import("../../src/app/language-detection.js");
  const { loadFrozenCorpora } = await import("../helpers/language-corpus.js");
  const { translateCorpusSample } = await import("../helpers/language-translation.js");
  const manifest = JSON.parse(readFileSync("tests/fixtures/languages/media.json", "utf8")) as {
    entries: Array<{
      file: string;
  ...
  const german = loadFrozenCorpora().calibration.samples.find(
    (sample) => sample.sampleId === "calibration-de-eleven",
  )!;
  const external = await translateCorpusSample(german, "ja", "en");
  const helper = await nativeLanguagePreparation();
  try {
    let epoch = 0;
    for (const entry of manifest.entries) {
      for (const ffIndex of [entry.ffIndex, 1]) {
        const prepared = await helper.preparation.prepare(
          {
            playerId: "native-language",
            mediaEpoch: ++epoch,
            localPath: resolve(entry.file),
            isNetworkResource: false,
          },
          {
            trackId: ffIndex,
            ...
            codec: entry.codec,
            ffIndex,
            ...(entry.tag ? { language: entry.tag } : {}),
          },
        );
        expect(prepared, entry.file).not.toBeNull();
        const provider = new RecordingProvider();
        const controller = new PlaybackController({
          playerId: "native-language",
          provider,
        ...
        controller.tick(prepared!.cues[0]!.startMs);
        expect(provider.requests).toEqual([]);
        const detection = new LanguageDetectionCoordinator();
        let result: unknown;
        await detection.start(
          {
            playerId: "native-language",
            mediaEpoch: epoch,
            trackIdentity: String(ffIndex),
            contentHash: prepared!.contentHash,
            cues: prepared!.cues,
          },
          (value) => {
            result =
              value.state === "reliable"
                ? { state: value.state, languageId: value.languageId }
                : { state: value.state };
            controller.setLanguageDetection(
              value.state === "reliable" ? { languageId: value.languageId } : value.state,
            );
          },
        );
        expect(result).toEqual(
          ffIndex === entry.ffIndex ? external.result : { state: "reliable", languageId: "en" },
        );
        if (ffIndex === entry.ffIndex)
          expect(prepared!.cues.map((cue) => cue.normalizedText)).toEqual(
            german.cues.map((cue) => cue.normalizedText),
          );
        controller.tick(prepared!.cues[0]!.startMs);
        await controller.whenIdle();
        ...
        expect(
          provider.requests.every(
            (request) =>
              request.profileId === "native-profile" &&
              request.profileRevision === 4 &&
              request.sourceLanguage === (ffIndex === entry.ffIndex ? "de" : "en"),
          ),
        ).toBe(true);
        expect(
          readdirSync(helper.directory).filter((name) => /^[a-f0-9-]{36}$/.test(name)),
        ).toEqual([]);
        controller.close();
      }
    }
