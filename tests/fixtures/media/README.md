...
本清单只使用不透明 ID。下列 018 样本由仓库短句合成或由开发者自制；自动化小样本可由 `source/` 中的文本重建，大型与宿主样本不提交 Git。实机证据只记录 ID、类别、cue 数、耗时、状态与结论。
...
022 的自然字幕派生媒体位于 `language/`，许可、对应表及重建步骤见[语言语料说明](../languages/README.md#媒体重建与手动加载)。这些媒体的字幕保留各来源的署名及许可。
