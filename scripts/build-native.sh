...
for HELPER in "$OUTPUT_DIR/subtandem-transport" "$OUTPUT_DIR/subtandem-subtitle-extractor"; do
  chmod 755 "$HELPER"
...
node -e 'const fs=require("node:fs"),c=require("node:crypto");const [out,a,b]=process.argv.slice(1);const hash=p=>c.createHash("sha256").update(fs.readFileSync(p)).digest("hex");fs.writeFileSync(out,JSON.stringify({"subtandem-transport":hash(a),"subtandem-subtitle-extractor":hash(b)},null,2)+"\n")' "$HASH_FILE" "$OUTPUT_DIR/subtandem-transport" "$OUTPUT_DIR/subtandem-subtitle-extractor"
