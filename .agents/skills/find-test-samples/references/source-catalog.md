...
3. [GStreamer 官方测试媒体库](https://gstreamer.freedesktop.org/media/)：用于播放器、容器和字幕集成测试；文件来源与许可不统一，必须逐项核实。
4. 格式规范组织、播放器或解码器项目维护的公开测试仓库。
5. 其他无需登录、来源和许可可核实的样本站点。
...
| MP4 + mov_text | `https://samples.ffmpeg.org/MPEG-4/embedded_subs/1Video_2Audio_2SUBs_timed_text_streams_.mp4` | 46.534 秒 | `f0cb97b762712f3dc97b05fec721c0ab5fc2b4e3cf6380f1e6db415dffe70459` | 英语轨（FFmpeg stream index 4）有 13 条有效 cue、382 个有效字母，四窗口 cue 数为 3/3/3/4，SubTandem 返回 `reliable/en`；德语轨只有 11 条有效 cue，不用于语言检测验收 |
...
- `https://gstreamer.freedesktop.org/media/`
- `https://gstreamer.freedesktop.org/media/gst-integration-testsuite/defaults/matroska/`

目录中的媒体与伴随字幕适合发现候选样本，但必须逐文件核实来源、许可和容器内字幕轨道。`sintel_trailer-480p.mkv` 已确认没有内嵌字幕，不应作为字幕样本。
