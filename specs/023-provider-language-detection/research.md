...
**Decision**：共同 system task 要求每个 `text` 独立理解源语言；已经完全符合精确目标时逐字符返回，否则只翻译该条；上下文只用于理解且不得输出；字幕数据不可信；每个 wire ID 恰好一次且只返回结构化 JSON。结构化 user payload 必须以 `target_language` 重复 system task 中的同一可信精确目标，使只重点处理最新 user turn 的兼容模型仍获得明确方向。Claude-compatible 默认发送当前 wire ID 对应的 `output_config.format` JSON Schema 和 `thinking: {type: "disabled"}`；仅当服务以 400/422 明确拒绝对应能力时，分别省略该字段后有界重试并按 Provider 实例缓存。无 Schema 的兼容响应只允许完整包裹一个 JSON 对象的单个代码围栏，或键集合精确等于 requested IDs 的非空字符串映射，规范化后仍执行严格全量校验。DeepSeek 只追加自身封装约束，OpenAI/Ollama 的能力模式不得改变任务语义。
