...
**来源与结构**：自然字幕来自 Elephants Dream、Cosmos Laundromat、Tears of Steel 和 Sintel；来源 URL、字幕贡献者、逐项许可、非重叠选段、真值及冻结摘要由[语料目录](../../tests/fixtures/languages/sources.json)和[验证记录](verification.md#语料冻结与-t001t002)负责。目标语言为 `en de hu it ru sv he fi es fr pt nl da cs pl el id ja zh fa`。七个指定回归匹配 FFmpeg 原始样本及提取摘要，但没有再分发许可证据；两个短轨来自 CC BY 的 IETF CELLAR `test5.mkv`。
