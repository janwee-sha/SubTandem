...
- **决策**：受支持正文候选无需置信门槛；`reliable` 表示可进入翻译资格判断的候选。范围外确认单独处理，证据不足则使用受支持最佳候选。
...
- **决策**：从规范正文建立位置索引，按内容形成片段，保留重复出现次数；每段归属语言后累计有效文字量，平票按语言 ID 的固定序确定。小轨全量，长轨按正文位置有界采样并保留代表区间权重。
