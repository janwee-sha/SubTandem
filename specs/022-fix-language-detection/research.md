...
**理由**：[6.2.0 包元数据](https://github.com/wooorm/franc/blob/6.2.0/packages/franc/package.json)指定 ESM 与 MIT。[检测源码](https://github.com/wooorm/franc/blob/6.2.0/packages/franc/index.js)将输入截为前 2048 个 UTF-16 码元，返回相对距离排名。实际 npm 包含 186 个模型条目、178 个不同输出代码，不能把多脚本条目数当独立语言数；超限输入须分片以保留后段证据。
...
**决定**：同时按有效独立 cue 数及正文码元预算决定全量保留或四区域采样；单个超长 cue 也保留前后证据。检测副本去重，同一正文最多贡献一次证据，不改变翻译条目。每次 franc 输入 ≤2048 码元且不切断代理对；证据区间及总预算见[检测契约](contracts/language-detection.md#正文输入与可靠性)。

以有效文字量和完整候选竞争判断证据归属，再按各语言可靠归属的 Unicode 字母码点数选择唯一最高者；未归属文字保留，不强配给整体第一名。混合轨不要求赢家超过 50% 或固定支持比例，同脚本与跨脚本适用同一规则。删除 `minimumScriptRatio`、`minimumSupport` 搜索轴及决策门禁；脚本仅供构造证据或校准分层，单候选也须证明可靠性。并列、不足及不可映射时保留未知，禁止根据样本 ID、文件名、标签或指定短语特判。

**理由**：cue 数、文字系统分布与模型分数均不等于语言文字量。对白及其译文分别提供语言证据，人名、无语义内容和罗马字歌词不能因文字充分或重复而形成可靠结论。归属仅用于轨道判断，不新增逐 cue 产品接口或翻译语义校验。

**执行约束**：Coordinator 执行实际有界采样/分类片段，在片段间让出事件循环；开始、片段前后及提交均检查 deadline 和身份。定时器无法抢占同步 franc，500 ms 兜底与小同步工作量必须同时验证。到期/异常只提交一次未知结果；失效后停止后续片段。
...
**来源与结构**：现有 Elephants Dream、Cosmos Laundromat、Tears of Steel 和 Sintel 材料作为已知回归/开发材料；来源及摘要由[语料目录](../../tests/fixtures/languages/sources.json)和[验证记录](verification.md#语料冻结与-t001t002)负责。T050 另行核验独立作品字幕的再分发许可、正文及语言覆盖，来源池未经逐项核验不视为已取得材料。七个指定回归匹配 FFmpeg 原始样本及提取摘要；两个短轨来自 CC BY 的 IETF CELLAR `test5.mkv`，其所属作品不进入校准。

**决定**：按规格澄清，将逐条采集、许可/真值核验和清单冻结作为实施首个前置任务，完成后才能校准及验收。≥20 种语言、每种 ≥20 个独立自然字幕轨道的开放许可冻结集及全部分层、质量门保留；七个 FFmpeg 指定回归单独采用[本地验收](contracts/local-regressions.md)，不要求正文入库，不计入 SC-001，不作为 T001 前置条件；公开替代语料不能替代其原始缺陷断言。

**冻结原则**：字段及版本用途见[数据模型](data-model.md#验收语料与冻结记录)。校准包含多个独立作品、相近语言、有效文字充分的负样本及超过 2048 码元的自然输入，按作品分组校准；留出集按完整作品及全部译本与全部开发材料隔离。混合层先按正文确定语义版本、语言归属和文字量，再运行检测；单语失败项不得重标或移出。已评估材料及两个指定短轨只报已知回归，不能冒充未参与开发的留出结果。

**理由**：样本取得及逐条核验是明确的可执行工作；规划固定其来源池、记录结构、隔离方法及门禁，实施用已纳入版本控制的正文和许可证明完成。开放许可集合缺少许可、真值或两个指定短轨时，该前置任务保持未验收，不能用合成模板、私人路径或降低指标补足。七个 FFmpeg 指定回归的缺失或失败单独保持 SC-002 未验收。

目录条数或合成正文不构成自然字幕验收证据。
