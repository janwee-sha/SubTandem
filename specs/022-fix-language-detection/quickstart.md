...
本指南在实现完成后执行；新增的语料及测试入口由后续任务交付。规划检查不代表任何产品指标通过。行为依据见 [检测契约](contracts/language-detection.md)、[翻译任务契约](contracts/translation-task.md)，避免在证据中记录凭据、endpoint、字幕、译文或原始响应。
...
实施首先完成 `tests/fixtures/languages/` 的[语料记录](data-model.md#验收语料与冻结记录)：

- 冻结验收集至少 20 种可识别语言，每种至少 20 个自然、正文独立的单语正样本，另含不可可靠判断的负样本；覆盖 FR-013 全部分层。校准集单独采集、按完整同源组隔离。
- 逐条核实字幕本身的再分发许可、署名、正文真值、哈希和选段；可平移选段时间起点，但保留原 cue 正文、相对时间及原范围记录。
- 指定回归 ID 固定为 `regression-de-11`、`regression-short-5`、`regression-short-6`、`regression-hu-19`、`regression-it-19`、`regression-ru-19`、`regression-sv-19`、`regression-en-38`、`regression-romaji-29`。清单的 `file` 指向对应 SRT/ASS；德语和英语分别保留约 332/651 字母特征。指定样本未取得许可或正文时保持未验收，不能按 cue 数新造正文冒充。
- 冻结同语言 case 集，覆盖已知同语言、未知同语言、混合条目、标点/大小写/空格、多行/内部空行、重复正文、中文简繁及 `pt-PT`。每条预先标注 `verbatim/translate`，固定当前任务实际发送的 `text`。

新增 `tests/contract/language-corpus.test.ts` 检查 manifest、受版本管理的文件及许可记录、哈希、去重/同源隔离、数量和分层，并通过生产 parser 核对 cue。正文真值及授权仍需单名开发者依据来源审查，脚本通过不能代替。
...
| 检查 | 必须报告的结果 |
| --- | --- |
| SC-001 | 正样本正确可靠数/正样本数 ≥95%；全部样本错误可靠数/全部样本数 ≤1%；负样本可靠数/负样本数 ≤1%；按语言、长度及现象分别报告正确、未知/不支持、错误可靠的计数和比例 |
| SC-002 | 全部指定回归符合预期；同一正文在正确/缺失/错误标签下结果变化为 0，变体不能重复计入样本分母 |
...
| SC-005 自动化部分 | 假时钟覆盖 deadline 前/等于/之后、同步异常和未完成异步步骤；终态只提交一次，超时全部转未知且自动调度翻译；所有会话边界至少重复 20 次，双窗口使用不同 Main/Global ID |
...
| 操作 | 预期 |
| --- | --- |
| 打开视频，通过 IINA 字幕菜单加载清单中 `regression-de-11` 和 5–6 cue 短轨，选择与正文不同的目标语言并开启 Translate | 识别正确，自动进入翻译，无源语言输入和额外点击 |
...
只记录非敏感样本 ID、耗时、状态和通过/失败；原样逐字符准确率由上一节程序验证，人工补充视觉和时间轴。超时精确计时及迟到检测由自动化覆盖，不要求用户用肉眼估算 500 ms。浏览器控制或 Computer Use 不属于本指南执行方式。

## 验收完成条件

语料前置任务、聚焦/全量测试、性能、编译、打包、四类真实服务及单人宿主步骤都必须各有证据；未完成项继续保留为可执行任务。README/多语言说明与实际请求行为同步核对，禁止将自动化通过当作 live 或宿主通过。
