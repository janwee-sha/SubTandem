...
**规格**：[spec.md](spec.md) | **轨道**：完整 SDD | **状态**：实施中；任务与验收见 [tasks.md](tasks.md)

## 摘要

使用标准 `franc` 替换 `franc-min`，在有界、去重的检测副本中归属语言证据，按 Unicode 字母码点数选择唯一主要语言。取消主要文字系统占比、固定语言支持比例、cue 数及匹配窗口数门禁；并列、不足及不可映射的出口见[检测契约](contracts/language-detection.md#正文输入与可靠性)。可靠源语言和未确定源语言均可进入当前服务的翻译流程；全部任务要求逐条遵循目标语言及显式变体，已符合时原样返回正文。
...
| 语料       | T050 在重新开发校准前固定多个作品的校准集、独立留出版本及混合语义层；已评估材料只作已知回归/开发，全部数量、分层及质量门保留 |
...
| 交付与人工成本   | 通过       | 通过   | 单名开发者执行明确步骤；真实服务及宿主分别取得实际验证证据                                                                                                                       |
...
实施依赖见 [tasks.md](tasks.md#阶段-7收敛)：T045 统一当前意图 → T050 冻结校准及独立留出材料 → T049/T046/T051 检测采样与语言归属 → T047/T052 未知准入 → T048/T053/T054 原样正文 → T055/T058/T056 交付准备 → T057 真实服务及宿主验收。各项同时完成原任务对应要求，不替代其验收。公共类型及 Main/controller 串行集成。
...
T050 必须提供未参与开发的 ≥400 个单语自然轨道，按完整作品及译本隔离；混合主语言单列指标，已知指定回归单列 SC-002，不稀释 SC-001。语料许可、真值或数量不足时暂停检测开发校准，不降低门禁。franc 覆盖不保证短轨及相近语言可靠，阈值必须实际满足 SC-001/002；模型可能改写合法同语言正文，SC-004 需四类真实服务证明。事件循环及宿主时序必须实际测量，不能凭定时器或 Node 微基准宣称满足 SC-005。

实施范围、依赖及验收工作以 [tasks.md](tasks.md) 为准。
